...
- **Saved email addresses and site names (identity assist) — on by default, switch off in Settings → "ইমেইল ঠিকানা মনে রাখা":** the App remembers up to eight addresses you finish typing in email fields, and up to eight site names (such as example.com) you type, (never in ordinary text, and never in password, one-time-code, or "no personalized learning" fields) so it can offer them as one-tap fills in other email fields. Switching the setting off deletes the saved addresses immediately. Completing a typed "@gm" to a common provider such as gmail.com uses a built-in list and stores nothing.
...
- **Settings switches** let you stop learning ("টাইপিং শেখা"), stop the personal dictionary, turn saved email addresses and site names off (which also deletes the ones already saved), or turn clipboard history off (which deletes it).
