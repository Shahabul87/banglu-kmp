...
/**
 * Hands the MSI to Windows Installer — in the only order that works.
 *
 * The 1.0.1→1.0.2 update on the user's laptop proved the old order wrong:
 * launching msiexec and THEN quitting the app is a race the app always loses.
 * Windows Installer's files-in-use check ran while বাংলু টাইপার was still
 * alive holding its own EXE, the jpackage runtime and the dictionary; the
 * installer could not auto-close us (the tray owns the lifetime — our windows
 * refuse WM_CLOSE on purpose), so it showed "setup was unable to automatically
 * close all requested applications" and scheduled the locked files for
 * replace-at-reboot. That schedule IS the "please restart your PC".
 *
 * So the install is now STAGED, not launched: a small PowerShell script is
 * written beside the MSI and started detached, and it
 *
 *  1. waits (max 60s) for THIS process id to exit — the caller quits the app
 *     immediately after staging;
 *  2. aborts if the app never exits, rather than reproducing the dialog;
 *  3. only then runs `msiexec /i … /qb REBOOT=ReallySuppress` — nothing of
 *     ours is running, so nothing is in use, so no reboot is ever scheduled
 *     (`/qb` keeps the progress bar; the suppress is belt-and-braces);
 *  4. on success (0, or 3010 which cannot happen with nothing locked)
 *     relaunches the freshly installed app, so the user watches the progress
 *     bar and then simply has বাংলু টাইপার back — no Start menu hunt.
 *
 * Windows-only by construction, exactly like `StartupRegistry` — the guard is
 * checked before anything is written or spawned so this repo's Mac test suite
 * can never stage anything. The script text itself is a pure function so the
 * whole contract above is pinned by tests that never spawn a process.
 */
internal object Installer {
    fun isWindowsOs(osName: String?): Boolean =
        osName?.contains("windows", ignoreCase = true) == true

    /** PowerShell single-quoted literal: the only metacharacter is ' itself. */
    private fun ps(literal: String): String = "'" + literal.replace("'", "''") + "'"

    /**
     * The staged updater, as text. [pid] is the running app (the script's
     * wait target), [msiPath] the verified download, [relaunchPath] the
     * installed launcher (`jpackage.app-path`) or null in a dev run.
     */
    fun script(pid: Long, msiPath: String, relaunchPath: String?): String = buildString {
        appendLine("# বাংলু টাইপার update runner — generated by the app, deleted on the next check.")
        appendLine("\$ErrorActionPreference = 'SilentlyContinue'")
        appendLine("\$deadline = (Get-Date).AddSeconds(60)")
        appendLine("while (Get-Process -Id $pid -ErrorAction SilentlyContinue) {")
        appendLine("    if ((Get-Date) -gt \$deadline) { exit 1 }")
        appendLine("    Start-Sleep -Milliseconds 250")
        appendLine("}")
        appendLine(
            "\$installer = Start-Process -FilePath 'msiexec.exe' -ArgumentList @('/i', " +
                "('\"{0}\"' -f ${ps(msiPath)}), '/qb', 'REBOOT=ReallySuppress') -Wait -PassThru"
        )
        if (relaunchPath != null) {
            appendLine("if ((\$installer.ExitCode -eq 0) -or (\$installer.ExitCode -eq 3010)) {")
            appendLine("    Start-Process -FilePath ${ps(relaunchPath)}")
            appendLine("}")
        }
        appendLine("exit \$installer.ExitCode")
    }

    /**
     * Writes the script beside [msi] and starts it detached. Returns false —
     * with the app left running and the offer intact — on any refusal or
     * failure; true means the runner is live and the caller must now quit.
     */
    fun stage(
        msi: File,
        pid: Long,
        ...
        return runCatching {
            val runner = File(msi.parentFile, "banglu-update.ps1")
            runner.writeText(script(pid, msi.absolutePath, relaunchPath))
            ProcessBuilder(
                "powershell",
                "-NoProfile",
                "-ExecutionPolicy", "Bypass",
                "-WindowStyle", "Hidden",
                "-File", runner.absolutePath,
            ).start()
            true
        }.getOrDefault(false)
    }
...
private const val LINE_INSTALLING =
    "আপডেট ইনস্টল হচ্ছে। বাংলু টাইপার এখন বন্ধ হয়ে ইনস্টল শেষে নিজে থেকেই আবার চালু হবে।"
...
class UpdateService(
    ...
    /**
     * Downloads, verifies, STAGES the installer (see [Installer]), and only
     * then calls [onInstallerStarted] — which the caller wires to quitting the
     * app. The staged runner waits for this very process to exit before
     * msiexec is allowed to start: install-then-quit was the bug (files in
     * use, reboot scheduled), quit-then-install is the fix.
     *
    ...
    override fun install(onInstallerStarted: () -> Unit) {
        ...
        try {
            ...
            val staged = Installer.stage(
                msi = target,
                pid = ProcessHandle.current().pid(),
                // The installed launcher's own path, stamped by jpackage; null
                // in a dev run, where there is nothing sensible to relaunch.
                relaunchPath = System.getProperty("jpackage.app-path"),
            )
            if (!staged) {
    ...
    /**
     * Removes MSIs — and the staged runner scripts beside them — left behind
     * by an earlier run. We exit as soon as the runner is staged, so nothing
     * else ever gets the chance to clean up, and a ~150MB file per update
     * would otherwise accumulate forever in `~/.banglu/updates`.
     */
    private fun prune() {
        runCatching {
            downloadDir.listFiles()?.forEach { file ->
                if (file.isFile && (file.name.endsWith(".msi") || file.name.endsWith(".ps1"))) {
                    file.delete()
                }
            }
        }
