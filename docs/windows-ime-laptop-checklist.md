...
lookup — has never executed on a real Windows machine. `:windows-ime:test` (63
...
| 0.4 | **Keyboard layout** in use (Settings → Time & language → Language & region → the input method under English/whatever language, e.g. "US QWERTY", "UK", "US International", a Bengali layout). Record it verbatim. The hook's punctuation table assumes **US**: Shift+1 = `!`, `/`?` on the OEM_2 key, `.` and `,` on OEM_PERIOD/OEM_COMMA. On a non-US layout rows 2.4, 2.5, 2.10 and 2.11 may produce different characters — that is a layout mismatch to record, not necessarily a bug. | | | |
...
| 2.16\* | Candidate chip click | Type `kmn` (or any word with multiple ranked candidates) — the preview strip shows candidate chips below the forming word. Click a non-first candidate chip. | That candidate's Bangla text is injected into the document (not the original top-ranked preview). Also eyeball the strip itself: up to **six** chips, numbered ১–৬, all fully visible and none clipped at the right edge — and the last chip is the roman text you typed (`kmn`), the escape hatch for words you want to leave in English. | |
| 2.16b | The sixth candidate is pickable by digit | Type `kmn` again and, while it is still forming, press `6` on the number row | The raw roman `kmn` is committed verbatim (not converted) — the same thing clicking the last chip does. Every visible chip must have a working digit and vice versa; if `6` instead types **৬** into the document, the chip and digit ranges have gone out of sync again. | |
...
| 2.18 | Ctrl-chords while a word is forming | Type `am` (do NOT finish the word — the preview strip is showing), then press **Ctrl+A**. Then type `i` and press Space. Repeat the same shape with **Ctrl+V** (paste something) and **Ctrl+Z** (undo) in place of Ctrl+A. | Known, accepted behaviour: any Ctrl/Alt/Win chord goes straight to the application while our buffer still holds `am`, so the chord acts first and the *next* letter then commits **আম** on top of whatever the chord did (e.g. after Ctrl+A, typing `i` replaces the selected text with আম-then-ই). **This row is about how it FEELS, not pass/fail** — record whether it was surprising, whether you lost text you cared about, and which chord you tried. If it feels bad in practice that is a real finding worth a design change. | |
| 2.19 | Caret moved by mouse click, mid-word | Type `am` (do NOT finish the word), then **click somewhere else in the same document** with the mouse, then type `i` and press Space | Known, accepted limitation: only a *foreground window* change flushes the composer, and a click inside the same window is not one — so **আম** commits at the NEW caret position, not where you were typing. Avro behaves the same way (it is inherent to a keyboard-hook IME that cannot see the host's caret move). Record how disruptive it felt and whether it happened to you accidentally during the 30-minute session in 5.7. | |
...
| 4.17 | Manual hook recovery | Tray menu → **"কীবোর্ড কাজ করছে না? আবার চালু করুন"** (this item is only enabled once the dictionary has finished loading) | After clicking it, typing in Notepad still converts correctly — the keyboard keeps working after a manual re-arm | |
| 4.18 | Passthrough app list, end to end | Quit বাংলু টাইপার (tray → **"বন্ধ করুন"**). Create/edit `%USERPROFILE%\.banglu\winime-appcompat.json` to contain exactly `[{"exe":"notepad.exe","passthrough":true}]`. Relaunch, wait for **"পূর্ণ অভিধান ✓"**, and type `ami` in Notepad. Then quit again, delete that file (or set `"passthrough": false`), relaunch, and type `ami` in Notepad once more. | With the row present, Notepad gets plain roman `ami` — no conversion at all — while typing in another app (e.g. WordPad or Chrome) still converts normally. After removing the row and relaunching, Notepad converts to **আমি** again. The restart is required by design (the list is snapshotted at startup) — confirm that editing the file *without* restarting changes nothing, since that is the trap a user will fall into. | |
...
| 5.9 | Enter right after a word, having switched windows a lot | Alt-Tab between three or four windows several times (at least five switches — click into other windows too, so real foreground changes fire). Then open a chat box (WhatsApp Desktop, or any Send-on-Enter field), type `ami`, and press **Enter immediately**, as fast as you can. Repeat five times. | The message must always contain **আমি**. The failure this row hunts: the Enter overtakes the pending word, the message sends empty (or without the last word) and **আমি** then appears in the now-empty box. That was a real bug — the alt-tabbing is what triggered it, because every foreground change used to corrupt the counter that orders these two events — and this row is what proves it stays fixed. Any occurrence, even one in five, is a Fail. | |
| 5.10 | Silent hook death — no tray warning | During (or after) the 30-minute session in 5.7: if Bangla conversion ever stops happening — keystrokes come out as plain roman — **and the tray shows NO "⚠ কীবোর্ড হুক বসেনি" warning** | This is the one failure the tray cannot detect: Windows can drop a hook it judged slow without telling anyone, and we still hold the handle, so the tray reads healthy while the keyboard is dead. First action: tray menu → **"কীবোর্ড কাজ করছে না? আবার চালু করুন"**, then type again. Record: what you were doing in the seconds before it stopped, which app had focus, whether the machine had just resumed/locked/switched user, whether anything heavy was loading, and whether the re-arm brought it back. If it happened at all, that is the single most valuable observation in this whole checklist — write it up in full. | |

---

## Summary

- Total checklist items: **68** individual pass/fail rows across sections 0–5
  (0: 4, 1: 9, 2a: 20, 3: 7, 4: 18, 5: 10), plus the six-app repeat sub-table
