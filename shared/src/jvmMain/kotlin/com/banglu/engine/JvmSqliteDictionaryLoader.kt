 ...
 */
open class JvmSqliteDictionaryLoader(
    ...
    private inline fun <T> withConnection(block: (Connection) -> T): T? {
        val conn = try {
            ...
            System.err.println("JvmSqliteDictionaryLoader: Failed to open database: ${e.message}")
            return null
        }
        return try {
            block(conn)
    ...
    override suspend fun loadFullDictionary(): List<String>? = withConnection { conn ->
        ...
        println("JvmSqliteDictionaryLoader: Loaded ${words.size} words from dictionary")
    ...
    override suspend fun loadFrequencyMap(): Map<String, Int>? = withConnection { conn ->
        ...
        println("JvmSqliteDictionaryLoader: Loaded ${freqs.size} frequency entries")
    ...
    override suspend fun loadDisambiguationMap(): Map<String, String>? = withConnection { conn ->
        ...
        println("JvmSqliteDictionaryLoader: Loaded ${map.size} disambiguation entries")
    ...
    override suspend fun loadExtendedDictionary(): List<SmartDictionaryEntry>? {
        return withConnection { conn ->
            ...
            println("JvmSqliteDictionaryLoader: Loaded ${result.size} extended dictionary entries")
            result.ifEmpty { null }
        }
    }
    ...
    override suspend fun loadBigramModel(): BigramModelData? {
        return withConnection { conn ->
            ...
            println(
                "JvmSqliteDictionaryLoader: Loaded ${unigrams.size} unigram entries, " +
